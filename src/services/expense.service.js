...
const create = async (userId, data, file) => {
  return sequelize.transaction(async (t) => {
    ...
    if (file) {
      payload.attachment_path = `/uploads/expenses/${file.filename}`;
      payload.attachment_name = file.originalname;
      payload.attachment_type = file.mimetype;
    }
    const expense = await repo.create(payload, { transaction: t });
...
const update = async (id, userId, data, file) => {
  return sequelize.transaction(async (t) => {
    ...
    if (file) {
      ...
      next.attachment_path = `/uploads/expenses/${file.filename}`;
